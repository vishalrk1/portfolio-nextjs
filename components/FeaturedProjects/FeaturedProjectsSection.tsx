...
const FeaturedProjectsSection = () => {
  return (
    <section id="projects" className="flex flex-col gap-8 justify-center bg-secondary w-full px-6 md:px-12 xl:px-40 py-8">
      ...
      {ProjectsData.filter((item) => item.isFeatured).map((item, index) => (
        ...
        >
          <div
            className={`relative aspect-video w-full h-1/2 top-12 md:top-0 ${
          ...
          >
            <h1 className="hidden md:block text-3xl font-bold px-2 text-white">{item.name}</h1>
            <div
              className={`bg-gray-900 text-white rounded-lg shadow-lg overflow-hidden flex h-max w-full`}
            >
              <div className="p-6">
              <h1 className="block md:hidden text-2xl mb-2 font-bold text-white">{item.name}</h1>
